*** Settings ***
Documentation	Tests for lists with new formatting (square brackets) - since eggPlant 20.1.0.
...
List with new line in the value
	...
	Log list	${expected}
	@{l}=	Lists. return list with new line in the value
	Should be equal  ${l}  ${expected}

List with single value
	@{expected}=	Create List   single_value
	Log list	${expected}
	@{l}=  Lists. return list single value
	Should be equal  ${l}  ${expected}
...
List With Unpaired Closure Bracket And Open Brackets In String Middle
	...
	Log list	${expected}
	@{l}=	Lists. return List With Unpaired Closure Bracket And Open Brackets In String Middle
	Should be equal	${l}	${expected}
...
Special Nested List
	...
	Log list   ${expected}
    ${l}=   Lists. return Nested List Special
    Should be equal  ${l}  ${expected}
...
Nested List With Empty Values
    ...
    Log list   ${expected}
    @{l}=  Lists. return nested list with empty values
    Should be equal  ${l}  ${expected}
...
Nested List With Empty Value At Start (Level 3)
    ...
    Log list   ${expected}
    @{l}=  Lists. return nested list with empty value at start
    Should be equal  ${l}  ${expected}
...
Nested List with a string value with brackets
	...
	@{expected}=	Create list  ${1}	${2}	${Level 2}	${3}
    Log list   ${expected}
    ${l}=  Lists. return nested list with string inside  	${Some string with last bracket}
    Should be equal  ${l}  ${expected}